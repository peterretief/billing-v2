    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            try:
                ...
                logger.error(
                    f"Tenant invitation failed for {form.cleaned_data['email']}. Error: {str(e)}", exc_info=True
                )
                messages.error(request, "An unexpected error occurred. Please check the logs or contact support.")
...
class OpsDashboardView(LoginRequiredMixin, UserPassesTestMixin, TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        if user.is_superuser:
            ...
            assigned_tenant_ids = OpsAssignment.objects.filter(ops_user=user).values_list("tenant_id", flat=True)
        ...
        revenue_by_currency = (
            Invoice.objects.filter(user_id__in=final_tenant_ids)
            .values(currency=F("user__profile__currency"))
            .annotate(total=Sum("total_amount"))
            .order_by("currency")
        )

        # --- STEP 4: BATCH PROGRESS (FOR THE PROGRESS BAR) ---
        batch_qs = BillingBatch.objects.filter(user_id__in=final_tenant_ids, scheduled_date=today)
