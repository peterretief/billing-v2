...
class CancelledInvoiceTotalsTest(TestCase):
    ...
    def test_cancelled_with_payments_excluded(self):
        ...
        Payment.objects.create(
            user=self.user,
            invoice=cancelled_inv,
            amount=Decimal('500.00'),
            reference='Partial Payment Before Cancel'
        )
...
class CancelledInvoiceFromAuditTest(TestCase):
    """Test that cancelling from audit report removes invoice from totals."""
    ...
    def test_invoices_cancelled_from_audit_excluded_from_totals(self):
        """Verify invoices cancelled via audit report are excluded from totals."""
        # Create first invoice with $500
        ...
        Invoice.objects.update_totals(invoice1)
        invoice1.status = 'PENDING'
        invoice1.save(update_fields=['status'])
        
        # Create second invoice with $300
        ...
        Invoice.objects.update_totals(invoice2)
        invoice2.status = 'PENDING'
        invoice2.save(update_fields=['status'])
        
        # Verify both are in outstanding
        outstanding_before = Invoice.objects.get_total_outstanding(self.user)
        self.assertEqual(outstanding_before, Decimal('800.00'))
        
        # "Cancel from audit" - set status to CANCELLED
        invoice1.status = 'CANCELLED'
        invoice1.save()
        
        # Verify only invoice2 is now in outstanding
        outstanding_after = Invoice.objects.get_total_outstanding(self.user)
        self.assertEqual(outstanding_after, Decimal('300.00'))
