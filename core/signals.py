...
def mark_profile_complete(sender, instance, **kwargs):
    ...
    if instance.company_name and instance.address:
        ...
        Notification.objects.filter(user=instance.user, message__icontains="profile", is_read=False).update(
            is_read=True
        )
...
def mark_timesheet_complete(sender, instance, created, **kwargs):
    ...
    """
    if created:
        Notification.objects.filter(user=instance.user, message__icontains="Step 4", is_read=False).update(is_read=True)
