...
class BillingPolicyForm(forms.ModelForm):
    class Meta:
        ...
        widgets = {
            "name": forms.TextInput(
                attrs={"class": "form-control", "placeholder": "e.g., Standard Monthly Subscription"}
            ),
            "run_day": forms.NumberInput(
                attrs={"class": "form-control", "min": "1", "max": "31", "placeholder": "1-31"}
            ),
            "is_active": forms.CheckboxInput(attrs={"class": "form-check-input"}),
        }
        labels = {"run_day": "Day of the Month to Run", "is_active": "Enable Automatic Invoicing"}
    ...
    def clean(self):
        ...
        if special_rule == "NONE" and not run_day:
            raise forms.ValidationError("Please either specify a Day of the Month or select a Special Rule.")
